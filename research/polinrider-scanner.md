...
| Two named repositories/accounts were infected; four local copies existed; the local payload was 5,533 bytes; no execution occurred on the inspected host; authors were forged; the attacker re-infected the environment four times; `flo-ct-flo360`, `178.156.147.109`, and a disclosed password are pivots/IOCs. | **Not independently reproducible from public campaign research.** These are case-specific assertions about a private host, repositories, identities, credentials, prior remediation attempts, and local evidence. The artifact alone cannot establish them. | Do not embed repo/account/person names, the cloud host, credential strings, or `flo-ct-flo360` in a general malware scanner. They belong only in an authenticated incident case record. The scanner also cannot determine past execution or exfiltration from a repository tree. |
